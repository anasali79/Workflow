...
async function handleWebhook(request: NextRequest, triggerIdFromPath?: string) {
  try {
    ...
    // 2. Resolve trigger ID (Path -> Query -> Body)
    let triggerId = triggerIdFromPath || request.nextUrl.searchParams.get("triggerId") || request.nextUrl.searchParams.get("trigger_id");
